...
var timetocheck = 3 + Math.floor(Math.random()*5); 
...
var styles = '.FreezePaneOff {\
	...
	.InnerFreezePane {\
		...
		color: White;\
		font-size: large;\
		border: dashed 2px #FF00AA;\
		padding: 9px;\
	}';

//----------------------------------------

function check_new(msg) {
	var url = "http://" + document.domain + "/dorf3.php";
	_(1, 'Checking by dorf3: '+url);
	send(url,function(responseDetails) {
			...
			if (checkImg(pulled)) {
                ...
                infobar.innerHTML ='<span style="color:#0000FF">No attacks found! Your lucky!!! =)</span>';
                FreezeScreen('', false);
...
function check(nr) {
	var url = "http://" + document.domain + "/dorf1.php" + params[nr];
	_(1, 'Checking '+nr+': '+url);
    send(url,function(responseDetails) {
			var pulled = document.createElement('div');
			pulled.innerHTML = responseDetails.responseText;
...
function last(nr) {
	if (active) {
		var temp = active.href.split("?")[1].split('&');
		//lastlink = temp[0];
		var url = "http://" + document.domain + "/dorf1.php?" + temp[0];
		...
		send(url, function(responseDetails) {
				...
				if (checkImg(pulled)) {
					...
					infobar.innerHTML ='<span style="color:#0000FF">No attacks found! lucky devil!!! =)</span>';
				} else {
					var msg ='<span style="color:#FF0000" title="'+fullInfo+'">' + villageNameA + ' Under attack!</span>';
					if (sounds_enabled)
						msg += "<embed src='"+soundUrl+"' hidden=true autostart=true loop=false>";
...
function checkImg(doc) {
	var tag = find(".//img[contains(@class,'att1')]",XPList, doc);
...
function mainFunc() {
	...
	//get userID
	var uLink = find("//div[@id='side_navi']//a[contains(@href, 'spieler.php')]", XPFirst);
	if (uLink) {
