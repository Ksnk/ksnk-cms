...
+ добавлена опция depend в теги XML.
 ...
 depend="*.js" - пересборка всех файлов, время сборки которых меньше максиммального из
 времен модификации файлов.
...
пропущены строки с секретным смыслом! Не помешает сделать так, чтобы длина заготовки совпадала
по количеству строк с длиной шапки так как в этом случае при отладке номер строки построенного файла будет совпадать с исходным.

--- затем в файл сборки проекта вставить несколько пропертей и таргетов
...
после этого в файле rev.tmp будет обработанная с помощью SVN шапка проекта.

-- добавить вот такое в config.xml проекта
    <files>
        <echo>
            <![CDATA[</% point_start('hat');include('rev.tmp'); point_finish() ;%/>]]>
        </echo>
    </files>
