...
class prop {
  ...
  */
   ...
   */
	static function sema($n,$s=' ',$last=false)     // $last - обязательно дописывать подпись
	{
	  	if(empty($this) 
	  	||  !($this instanceof prop))
	  		$me=prop::instance();
	  	else
	  		$me=&$this;	
	   	if(!is_array($s)){
  ...
  */
	static function num2str($LL,$valute=FALSE,$kop=FALSE)
	{
