...
//        else if (is_string($arg))$canget=$arg;
	...
	function scanHtml($html){
		...
		// начало
		//debug(strlen($html));
		if (($htlen=strlen($html))>90000){
			$found=preg_match('~<form\b([^>]*)name=[\'"]'.$this->name.'[\'"]([^>]*)>~ims',substr($html,0,1000),$m);
			$found=$found && preg_match('~</form>(.*)$~ims',substr($html,90000),$mm);
			//error_log($htlen.' "'.$mm[0].'"',3,'z:/logs/debug.log');
			$m[3]=substr($html,strlen($m[0]),$htlen-strlen($m[0])-strlen($mm[0]));
		} else {
		...
		if($found){
			...
			$this->method=strtoupper(pps($x[0]));
			$this->action=pps($x[1]);
			$this->par=pps($x['par']);
			// сканируем контролы
			while(preg_match(
				...
				, $m[3],$x))
			{
				...
				$x[2]=strtolower($x[2]);
				if ($x[2]=='input'){
					$y=scanPar($x[3],array('type'=>0,'name'=>1,'value'=>2,'checked'=>3));
					$y[0]=pps($y[0]);
					if(($y[0]=='button')||(empty($y[1]))) {
						...
						$y['values']=(isset($y[3])?'1:':(!empty($y[2])?$y[2].':':'')).trim($z[1]);
						//*-----*/print_r('!!!!!!!!!!'.htmlspecialchars($z[1]));
						$m[3]=substr($m[3],strlen($z[0]));$x[0]='';
						$y[2]=$y[0];
						$y[0]=CNT_INPUT ;
			...
			if (!empty($_FILES))
			{
				if(isset($_FILES['Filedata'])){ 
					...
					if ($f['error'] == UPLOAD_ERR_OK) {
						//echo('xxx'.$k);
						$tmp_name = $f["tmp_name"];
						if(detectUTF8($f["name"])){
							$f["name"]=iconv('utf-8','cp1251//IGNORE',$f["name"]);
						}
