<% point_start('history');%>
История изменений проекта Jinja4PHP

фильтр in_array

-- добавлены функции
range - range(N) - массив от 0 до N-1
keys - массив ключей ассоциативного массива
slice - разбивка массива на группы по Х элементов

~~ из-за особенностей реализации - все имена параметров макрокоманд при вызове макрокоманды 
с помощью массива параметров, следует указывать в нижнем регистре

0.8 - первый релиз проекта
<% point_start('hat');
// шапка для вставки в файлы проекта
%>
----------------------------------------------------------------------------
$Id:Jinja4PHP. Jinja2 language template, written by Ksnk (sergekoriakin@gmail.com)
Ver : 0.8(beta), Rev: <%=$SVN_Revision%>, Modified: <%=$SVN_Date%>$
----------------------------------------------------------------------------
...
==Читаем Jinja2 документацию,для описания имеющихся отличий

Изменением, по сравнению с Jinja синтаксисом, будет возможность указывать ключи данных в `` кавычках (обратная одинарная). Такой синтаксис позволит добавить возможность использования ключей с пробелами и странными символами в параметрах шаблона. Если в ключе параметра используется символ `, его нужно escape’ить по обычным правилам языка С.

Изменением будет также дополнение тега IF. Вместе с конструкцией elif можно будет указывать его синоним elseif, чтобы избежать ненужных ошибок у php програмистов.
...
Именованые теги окончания блоков не реализованы.
...
В результате работы транслятора получается некий php-класс. ПРи рендеринге данных достаточно проинициировать этот класс и вызвать либо нужную функцию, либо “корневую” - ‘_’. Никаких дополнительных действий и включения в код файлов компилятора не обязателен. Дополнительные фильтры и тесты? использующиеся в run-time рендеринге, описываются в классе tpl_base. 
Каждый блок шаблона транслируется в функцию класса. Весь шаблон транслируютется a функцю ‘_’. Макрокоманды являются такими-же функциями класса, однако в конструкторе класса заводится массив macro со всеми описанными в шалоне макросами для последующего импора. Импорт макросов заключается в инициализации импортируемого класса шаблона и слиянии собственного массива macro с импортируемым. "Переопределение" блоков достаточно корректно эмулируется простым наследованием классов. При этом отсутствует возможность эффективно узнать структуру предка класса и наличие импортируемых макросов. Таким образом, вставка макроса выглядит так
...
->newOp1('not','!(%s)','BB')
Первый параметр  - имя операции, второй printf-шаблон для генерации кода, третий - указания по преобразованию типов. Первая литера - преобразование к логическому типу, вторая литера - преобразование операнда к логическому типу перед вставкой в шаблон. Бинарные операции вводятся оператом newOp2
->newOp2('== != > >= < <=',2,null,'B**')
...
- особый оператор lo  op. В зависимости от  применяемых к loop атрибутов  получается та или иная порождаемая конструкция. Вид и смысл конструкции определяется callbacl'ом.   
...
Для поставки достаточно сстемы рендеринга и оттранслированных 'боевые' шаблоны
