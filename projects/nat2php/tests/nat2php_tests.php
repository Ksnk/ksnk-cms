...
class TestOfExistans extends UnitTestCase {
    ...
    function testCompilation1() {
        $s='1';
		$this->assertEqual($this->run_script($s),1);
	}
    function testCompilation2() {
        $s='1+1';
		$this->assertEqual($this->run_script($s),1+1);
	}
    function testCompilation3() {
        $s='1+1*2';
		$this->assertEqual($this->run_script($s),1+1*2);
	}
    ...
    function testCompilation5() {
        $s='1*1+2^2';
		$this->assertEqual($this->run_script($s),1*1+pow(2,2));
	}
	function testCompilation6() {
        $s='pi/2';
		$this->assertEqual($this->run_script($s),pi()/2);
	}
	function testCompilation7() {
        $s='sin(pi/2)';
		$this->assertEqual($this->run_script($s),sin(pi()/2));
	}
	function testCompilation8() {
        $s='sinx(pi/2)';
		$this->expectException();$this->run_script($s);
	}
	...
	function testCompilation11() {
        $s='x+1';
		$this->assertEqual($this->run_script($s),2);
	}
	function testCompilation12() {
        $s='-x+1';
		$this->assertEqual($this->run_script($s),0);
	}
	...
	function testCompilation14() {
		...
		$this->assertEqual($this->run_script($s),234+234*2323*(3+4)+pow(2,3+4));
	}
	function testCompilation15() {
        $s='tan(x)+x+4*x^3+2*x+300';
		$this->assertEqual($this->run_script($s,1.34),tan(1.34)+1.34+4*pow(1.34,3)+2*1.34+300);
	}
	...
	function testCompilation19() {
        $s='x++ + ++x';
        $x=3;
		$this->assertEqual($this->run_script2($s,3),$x++ + ++$x);
	}
  ...
  function testCompilation21() {
    $s='1+2+3';
    $x=3;
    $this->assertEqual($this->run_script2($s,3),1+2+3);
  }
}

	$test = &new TestSuite('testing calculator engine');
    $test->addTestCase(new TestOfExistans());
