...
----------------------------------------------------------------------------
\$Id:$ID, 
Ver : $VERSION, Rev: $SVN_Revision, Modified: $SVN_Date $
----------------------------------------------------------------------------
"%>*/
...
//<%="\r\n".point('admin_includes'); %>

class ajax extends plugin
{
	
	function do_ajax_sendmail(){
		if(!$this->has_rights(right_WRITE))
			return "have no right? Sorry pal...";
		...
		$fio=pps($_POST['fio']);
		$body=pps($_POST['body']);
	// TODO: помирить этот код с функциональностьтью loadClass
		$mail=new html_mime_mail(
			//html_mime_mail::mail_header('From: ',$from,$fio).
			html_mime_mail::mail_header('Reply-To: ',$from,$fio).
			html_mime_mail::mail_header('Cc: ',$from,$fio) .
		    pp($cc1,'Cc: ',"\r\n") .
			pp($cc2,'Cc: ',"\r\n") .
			pp($cc3,'Cc: ',"\r\n") .
			pp($to,'Reply-To: ',"\r\n").
			pp(gmdate("j M y H:m"),'Date: '," GMT\r\n").
	...
	function do_ajax_insertArticle(){
	// elem - куда вставляем элемент
	// id - какой элемент вставляем
//	error_log('insert_Article',3,'z:/logs/debug.log');
		if(!$this->parent->has_rights(right_WRITE)){
			$this->parent->error('Нужно авторизоваться!!!!!');
			return ' ';
		}
		if(empty($_POST['elem']) || empty($_POST['id']))
			$this->parent->error('Ошибка в параметрах!!!!!');
		else {	
			$res=$this->parent->nodeGet($this->parent->nodeScanId($_POST['id']));
			$i=0;
			if(!($element=readElement($res,$i))) {
//	error_log('no_element',3,'z:/logs/debug.log');
				echo "Нету элемента!"; return ' ' ;
			};
			if(method_exists($element,'insertInto')){
//	error_log('calling',3,'z:/logs/debug.log');
				$element->insertInto($_POST['elem']);
			}
		}
		return ' ';
	}
	
	function do_ajax_insertElement(){
	// elem - куда вставляем элемент
	// id - какой элемент вставляем
//	error_log('insert_Article',3,'z:/logs/debug.log');
		if(!$this->parent->has_rights(right_WRITE)){
...
"nissan.png");
	...
	function do_ajax_moveNodes(){
		$nodex=$this->parent->nodeScanId($_REQUEST['x']);
		if($_REQUEST['y']=='p_0'){ // insert before all elements
			$items=$this->parent->nodeGetInfo($nodex);
			//print_r($items);
			$this->parent->MoveNode($nodex,-$items['pid']);
			return ' ';
		} else {
			$nodey=$this->parent->nodeScanId($_POST['y']);
			if($nodex>=0 && $nodey>=0){
				$this->parent->MoveNode($nodex,$nodey);
				return ' ';
			} else {
				return 'fail!';
			}
	...
	function do_file_uploader (){
		if(!$this->parent->has_rights(right_WRITE)){
			$this->parent->error('please login first!!!!!');
			return ' ';
		}
		$this->parent->sessionstart();
...
//		ob_start();
       ...
       //	echo'Hello!'; print_r($_FILES) ;
        ...
        if($form->handle()){
			...
			if(empty($x)) return 'unable to load files!';
			$x=pps($x[0]);
			if(basename($x)!=$form->files[$x]){
				@unlink($x);
				$x=dirname($x).'/'.$form->files[$x];
			}
			// получаем параметры для изменения
			$xid=ppi($form->var['use']);
			if($xid){
				$iter=2;
				do{
					$row=$this->parent->readRecord(array('id'=>$xid));
					//debug_log('xid',$xid,$row);
					if(!$row || $row['type']==type_NEWTEXTPIC || $row['type']==type_GALLERY) break;
					if(!$node=$this->parent->nodeScanId($xid)) break;
					//debug_log('node:',$node);
					if(!($row = $this->parent->nodeParent($node))) break;
					if(!$xid=$row['page']) break;
				} while($iter-->0);
				if(isset($row) && !empty($row['type'])){
		        	$w=ppi($row['sizex']);
					$h=ppi($row['sizey']);
					$xw=ppi($row['bsizex']);
					$xh=ppi($row['bsizey']);
					echo "use:";print_r($row);
				}
			}
			$row=$this->parent->export('defaultValues','defVal',type_NEWTEXTPIC);
			if (empty($w)) $w=ppi($row['sizex']);
			if (empty($h)) $h=ppi($row['sizey']);
			if (empty($xw)) $xw=ppi($row['bsizex']);
			if (empty($xh)) $xh=ppi($row['bsizey']);
			echo "default:";		print_r($row);
			
        	if (empty($w)) $w=ppi($this->parent->getPar('pictute_xwidth'),100);
			if (empty($h)) $h=ppi($this->parent->getPar('pictute_xheight'),80);
			if (empty($xw)) $xw=ppi($this->parent->getPar('pictute_xxwidth'),800);
			if (empty($xh)) $xh=ppi($this->parent->getPar('pictute_xxheight'),600);
...
group by x.id",$v);
		...
		if (isset($_POST['clear_empty_flesh'])){
			...
			$y=$this->database->selectCol(
				"SELECT distinct `id` FROM ?_flesh ".
				"WHERE `name`='root' and `sval`='defVal';");
			$x=array_diff($x,$y);
			if(!empty($x))
				echo implode(',',$x);
		}
...
/*<% if(!empty($auto_plugins)) echo "*"."/.',".implode($auto_plugins,",")."'/*"%>*/
/*<% if(!empty($auto_admin_plugins)) echo "*"."/.',".implode($auto_admin_plugins,",")."'/*"%>*/
		;
 ...
 */
	function do_siteParam($param=''){
		...
		'error'=>pps($_SESSION['errormsg']),'list'=>'');
		$res['list']=array();
		if ($param!='')
	...
	function _menu($skip=1){
		//debug(1111111111111111111);
		$x=$this->ffirst('getSiteMap');
		$res='';
		foreach(array('main'=>'<img style="position:relative;top:1px;left:8px;" src="img/razdels.gif">','catalogue'=>'Каталог') as $k=>$v){
			if($y=$x->scan($k)){
				//debug($y->getUlLi(3,false,1));
				$res.=smart_template(array(ELEMENTS_TPL,'sitemap'),
					array('id'=>$y->v['id'],'data'=>$y->getUlLi(1000,false,$skip),'menu'=>str_replace('%id%',$y->v['id'],$v)));
			}
		}
		//debug($res);
		return $res;
...
//debug($_SESSION['clipboard']);
	...
	//	$rsd=$this->database->selectCell('select `id` from ?_tree where `page`=?d',$rsd);
		...
		$form->scanHtml(smart_template(array('tpl_elements','_menuedit'),$res));
		if($form->handle()){
			if(!!$article){
	...
	function _xsearch(){
		...
		//debug('xseach');
			...
			$ss=strtolower(trim($_POST['adminsearch']));
			
			$_SESSION['search_string']=$ss;
			if ($_GET['id']=='fileman') {
				$_SESSION['search_id']='поиски в разделе Файлы';
				$this->parent->export('search','saveres',
				//array_merge(
					$this->parent->export('fileman','search',$ss)
				//)
				);
				
			} elseif ($_GET['id']=='users') {
				$_SESSION['search_id']='поиски в разделе Пользователи';
				
				//debug($ss);	
				$this->parent->export('search','saveres',
				//array_merge(
					$this->parent->export('users','search',$ss)
				//)
				);
			} else {
				$_SESSION['search_id']='поиск в разделе Каталог товаров';
				$this->parent->export('search','saveres',
				array_merge(
			...
			$this->parent->go('?do=search'.pp($_GET['debug'],'&debug=','','').$adv);
		}
		$this->parent->go($this->curl());
	}
	/**
	...
	function do_menu(){
		...
		if(!!$item){
			...
			/**
			 * префильтр всякой муры
			 */
			
			$x=$item->getContent();
			//debug($item->article);
			$res['has_menu']=array_merge($item->article->v,$item->v);
			$res['has_menu']['aid']=$item->article->v['id'];
			$res['has_menu']['id']=$item->v['id'];
			$item->serialize($res['has_menu'],false,1);
			$item->article->serialize($res['has_menu'],false,1);
			//debug($res['has_menu']);
	...
	function do_Default(){
		...
		if(!$this->has_rights(right_WRITE))
			return $this->ffirst('_loginform');
		//if(is_callable(array($this,'_xsearch')))$this->_xsearch();
		if(!defined('NONEWS'))
	...
	function do_search(){
		...
		if(!empty($_REQUEST['search'])){
			...
			if(!empty($_REQUEST['search_string'])){
				...
				$rr=array();
				if(class_exists('basket')){
					$rr=$this->export('katalog','search',$ss);
				};
...
if(isset($_GET['debug'])){
	if(empty($_GET['debug'])){
		setcookie("debug", "", time() - 3600,ROOT_INDEX);
	} else {
		setcookie("debug",$_GET['debug'],time()+3600,ROOT_INDEX);
	}
	$engine->go($engine->curl('debug'));
}
if(!empty($_COOKIE['debug']) && !isset($_GET['ajax'])){
	function backtrace(){
		$x=debug_backtrace();
		$z=array();
		$y=array_shift($x);
	...
	if (class_exists('Debug_HackerConsole_Main') ) {
		...
		function debug()
		{
			foreach(func_get_args() as $v){
				call_user_func(array('Debug_HackerConsole_Main', 'out'),$v);
			}
...
/*		function debug_log(){
			foreach(func_get_args() as $v){
				error_log(print_r($v,true),3,'z:/logs/debug.log');
			}
			error_log('>>>>>'.print_r(backtrace(),true),3,'z:/logs/debug.log');
		}
*/
$engine->execute('init');
...
// 	error_log(print_r($_POST,true),3,'z:/logs/debug.log');
if(isset($_POST['ajax'])){
 // декодируем utf8
 	foreach($_POST as $k=>$v){
 		if(detectUTF8($v)){
			$_POST[$k]=iconv('utf-8','cp1251//IGNORE',$v);
 		}
 	}
 	foreach($_REQUEST as $k=>$v){
 		if(detectUTF8($v)){
			$_REQUEST[$k]=iconv('utf-8','cp1251//IGNORE',$v);
 		}
			//	error_log(print_r($_POST,true),3,'z:/logs/debug.log');	
 	}
}
...
if(!empty($_POST['form'])){
	$m=explode(':',$_POST['form'].'::');
	$engine->export($m[0],'_'.ppx($m[1],'handle'));
//debug($m[0].'::_'.ppx($m[1],'handle'));
}

if(!$engine->has_rights(right_WRITE)){
	unset($engine->menu['right']);
	$engine->tpl='tpl_login';
}
