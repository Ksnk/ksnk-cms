...
/* throw template filled by values */
		...
		function _tpl($name,$method,$par=array()){
			static $cache;
			if(is_array($name)){
				if($method=='_'){
					$method='_'.$name[1]; $name=$name[0];
				} else {
					$name=$name[0];
				}
			}
			if(empty($cache[$name])){
				...
				if(!class_exists($name))
					return sprintf('method %s::%s not found',$name,$method);
				$cache[$name]=new $name();
			}
...
class prop {
   ...
   * @return prop
   */
  <% if (empty($php4)){ %>private <%}%>function prop() {
  ...
  */
	<% if (empty($php4)){ %>static <% } %>function prep($ss=' ') {
		$res=array();
		$ss = func_get_args();
   ...
   */
	<% if (empty($php4)){ %>static <% } %>function sema($n,$s=' ',$last=false)     // $last - обязательно дописывать подпись
	{
	  	if(empty($this) 
	  	<% if (empty($php4)){ %>||  !($this instanceof prop)<%}%>)
	  		$me=prop::instance();
	  	else
	  		$me=&$this;	
	   	if(!is_array($s)){
  ...
  */
	<% if (empty($php4)){ %>static <% } %>function num2str($LL,$valute=FALSE,$kop=FALSE)
	{
