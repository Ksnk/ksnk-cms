...
class tpl {
	...
	/**
	 * поддержка простых фильтров {VAR>>format}
	 */
...
//		echo('\n\rxxxxxxxxxx:'.$contents);
			...
			if ($rres){
				...
				$function[]='tpl::_a($par[\''.preg_replace('/ё\d+$/','',$mmm).'\'],array(\''.$file.'\',\''.pp($index,'','_').$mmm.'\'))';
			else if(isset($m[4])){		
