...
/**
 * отдельная модель для хранения корзин и истории заказов для пользователей в сессии
 */
class CGoodsStore {
    /**
     * зачистить текущую корзину
     * @return void
     */
    ...
    /**
     * поменять количество заказанных итемов
     * @param  $idX
     * @param int $numb
     * @param string $par
     * @return
    ...
    /**
     * установить товар по индексу в корзине
     * @param  $idx
     * @param  mixed $value
     * @return void
     */
...
/**
 * модель для хранения корзин в базе данных
 * для каждой сессии заводится basket_id, который хранится у юзера в куках вечно
 * итого - каждый юзер может собирать свою корзинку медленно и за несколько дней
 *
 * в базе корзинок по basket_id завязывается содержимое корзинки - сериализованный массив.
 * К корзинке приписывается, также идентификатор юзера.
 */
class CGoodsDbStore extends CGoodsStore {
    /**
     * зачистить текущую корзину
     * @return void
     */
    ...
    private $user,$basket_id;


    /**
     * конструирование объекта и получение первичных данных
     */
    function __construct(){
        if(empty(self::$db))
            self::$db=DATABASE();
        $this->basket_id=pps($_COOKIE['basket_id']);
        $this->user=pps($_SESSION['USER_ID']);
    }
    ...
    function clear(){
        if (!empty($this->basket_id))
            self::$db->query('delete ?_basket where basket_id=?',$this->basket_id);
    }
...
class basket extends ml_plugin {
    ...
    static public $store_class='CGoodsStore';
    ...
    /**
     * выдать содержимое корзины
     * @param int $subcat
     * @return array
     */
    function basket_items($subcat=1){
        $x=array();$basket=basket::getStore()->get();
        if (!empty($basket)){
	...
	function do_basket() {
        ...
        if($this->inumber()){
			...
			$katalogue=ClassByType(type_KATALOG);
            $v=$katalogue->getText($keys,$this->basket_data(),'Пересчитать');
        }
        return $v.$this->parent->_tpl('tpl_jelements','_basket_btn',$this->recalc());
	}

	function _basket() {
		return $this->parent->_tpl('tpl_jelements','_basket_top',$this->recalc());
	}
	
	function replaceItem($idX,$numb=1,$par=''){
        return basket::getStore()->replaceItem($idX,$numb,$par);
	}
	...
	function addItem($id,$numb=1,$par=''){
		...
		//debug('addItem '.$id);
		if (empty($numb))
            basket::getStore()->setItem($idX,null);
		else {
            $good=array(
				'id'=>$id,
				'n'=>$numb,
			...
			if(!empty($par))
				$good['par']=$par;
            //print_r($good);
            basket::getStore()->setItem($idX,$good);
		}
	}
...
class csv extends ml_plugin {
	...
	// анализ заголовков
		...
		while(!$csv_reader->eof() ){
			...
			if(!empty($key['pic_small'])){
				...
				if(!empty($names)){
					...
					$key['pic_small']=toUrl(TMP_DIR.$key['pic_small']);
					$key['pic_big']=$key['pic_small'];
				} 
				//debug($v);
		 ...
		 */
      // Эта функция ставит все остатки в единицу. Зачем была нужна, непонятно.
      // Я её закомментировал.
	...
	*/
			 ...
			 */
				// Сначала находим и удаляем связанные страницы с подробным описанием
				$res=$this->database->select(
					'SELECT ?_katalog.*
					FROM ?_katalog LEFT JOIN ?_temp ON ?_katalog.articul = ?_temp.articul
					WHERE (?_temp.articul IS Null) and (?_katalog.xarticle In ('.$articles.')) AND ?_katalog.the_href > 0;'
				);
				foreach($res as $k_res => $v_res) {
					$node = $engine->nodeScanId($v_res['the_href']);
					$engine->nodeDelete($node);
				}
				//Потом удаляем сами лишние элементы каталога				
				$res=$this->database->select(
					'DELETE ?_katalog.*
					FROM ?_katalog LEFT JOIN ?_temp ON ?_katalog.articul = ?_temp.articul
		...
		foreach($pattern as $v)
			if(!empty($v) && !in_array($v,array('articul','id','haract','article')))
				$update[]=sprintf('x.%s=y.%s',$v,$v);

		if(!empty($update)){
			$sql="UPDATE ?_katalog as x LEFT JOIN ?_temp as y ON RTRIM(REPLACE(REPLACE(x.articul,'\\r\\n',''),'\\t','')) = y.articul SET ".
		...
		if($havearticle) {
			...
			foreach($pattern as $v)
				if(!empty($v) && !in_array($v,array('id','article')))
					$insert[]=sprintf('%s',$v);
...
*/
	...
	function handlePost(&$res, $parent_id = 0){
		if(!empty($res)){
			$order=array();
			foreach($res as $v){
			...
			if(!empty($order)){
				if(in_array($this->cat,$this->specs)){
					...
					'(@order:=@order+1) as `num` from ?_katalog where `parent_id`='.$parent_id.' AND'.
					' `'.$sortby.'`>0 order by `'.$sortby.'`;';
				} else {
					$sortby=$this->sort;
					$sql='select `id` AS ARRAY_KEY,'.
					'(@order:=@order+1) as `num` from ?_katalog where `parent_id`='.$parent_id.' AND `xarticle`='.$this->cat.
					' order by `'.$sortby.'`;'; 
				}
				// перенумеровать
				//set @order=0;
				...
				if(!empty($sort)) {
					/*echo  'update ?_katalog set `'.$sortby.'`=field(`id`,'.implode(',',$sort)
		...
		if(isset($_POST['doIt'])){
			if(!empty($_POST['doIt'][1]))
				$_POST['sel_act']=pps($sel_act = $_POST['sel_act'][1]);
			else
				$_POST['sel_act']=pps($sel_act = $_POST['sel_act'][0]);
	...
	function get_sql($cat, $parent_id = 0){
		switch($cat){
			case 'null':
			...
			default: // вывод категорий!
				$sql='select * from ?_katalog where parent_id='.$parent_id.' AND `xarticle`='.$cat.
				(pps($this->sort)
					?' order by `'.$this->sort.'` '
					:' order by `articul` ');
		}
	...
	function data($what,$from='',$perpage=''){
		...
		//echo($what.' '.$from.' '.$perpage);
		$spec=false;
      //print_r($this);
      //if(!isSet($this->cat))
      	//$this->cat=pps($cat,$from);
      if($what == 'row') {
      	$sql = "SELECT * FROM ?_katalog WHERE id=".$from;
      }
      else {
		...
		switch($what){
			case "row":
				return $this->database->selectRow($sql);
			case "cnt":
			...
			case "data":
				...
				$head_items=array();
				foreach($yy as $k=>$v){
					$yy[$k]['subkat'] = $this->database->query($this->get_sql($this->cat, $v['id']));
					// Количество дополнительных характеристик
					$yy[$k]['subkat_cnt'] = count($yy[$k]['subkat']);
			...
			case "upd":
				...
				if(isset($from['articul'])) {
					//Чтобы убрать лишние табуляторы и переводы строки из артикула
					$from['articul'] = trim($from['articul']);
...
/*
			...
			where parent_id = 0 AND visibility = 0 AND ?_katalog.xarticle='.$cat.$where.
				($this->sort
					?' order by `'.$this->sort.'` '
					:' order by `articul` ');
/*
 ...
 */
		...
		if($x['parent_id'] == 0 && $per) {
			foreach($x as $k_sk => $v_sk) {
				$sql_sk='select ?_katalog.* from ?_katalog
				where parent_id = '.$x[$k_sk]['id'].' AND visibility = 0 AND ?_katalog.xarticle='.$cat.$where.
					($this->sort
						?' order by `'.$this->sort.'` '
						:' order by `articul` ');
				$x[$k_sk]['subkat'] = $this->database->query($sql_sk);
				if(empty($x[$k_sk]['subkat'])) {
					$x[$k_sk]['subkat'] = false;
					$x[$k_sk]['subkat_s'] = false;
					$x[$k_sk]['s_data'] = Array('name' => $x[$k_sk]['name'],
														'cost' => $x[$k_sk]['cost'],
														'ostatok' => $x[$k_sk]['ostatok'],
														'id' => $x[$k_sk]['id'],
														'value' => $x[$k_sk]['value']);
				}
				else {
					$x[$k_sk]['subkat_s'] = true;
					$x[$k_sk]['s_data'] = false;
					$x[$k_sk]['subkat_h'] = Array('id' => $x[$k_sk]['id']);
				}
				$x[$k_sk]['url'] = $_GET['url'];
				// Определяем, показывать ли ссылку на страницу с подробным описанием
				$node_href = $engine->node($x[$k_sk]['the_href']);
				if($node_href > 0) {
					$ch = $this->database->query("SELECT * FROM ?_tree WHERE pid = ".$node_href);
					if(empty($ch))
						$x[$k_sk]['show_podr'] = false;
					else
						$x[$k_sk]['show_podr'] = true;
				}
				else {
					$x[$k_sk]['show_podr'] = false;
				}
		...
		if(preg_match('/basket\d+/',$cat)){
            $basket=basket::getStore()->get();
 			foreach($x as $k=>$v){
				...
				if(!empty($basket[$v['xid']]['par'])){
					$x[$k]=array_merge($x[$k],$basket[$v['xid']]['par']);
				}
				if(empty($v['pic_small']))
					$x[$k]['pic_small']=$basket[$v['xid']]['pic_small'];
				$x[$k]['xcost']=$basket[$v['xid']]['xcost'];
				$x[$k]['cnt']=$basket[$v['xid']]['n'];
				$x[$k]['xid']=$v['xid'];
				//
				//$x['id']=$x['xid'];
			}	
		...
		if(strtolower($_SERVER['REQUEST_METHOD'])=='post'){
			...
			if(isset($_POST['clear_bsk'])){
				...
				foreach($_POST as $k=>$v){
					if((!empty($v) || ($v==='0') || ($v===0)) && preg_match('/^(x?item)_(-?\d+)$/',"x".$k,$m)){
						//debug($param[$m[1]]);
		...
		if(!empty($x))
			foreach($x as $k=>$v){
				...
				foreach(array('cost','cost1','cost2','cost3') as $vv) {
					...
					if(isset($x[$k]['s_data'][$vv]))
						$x[$k]['s_data'][$vv] = number_format($v[$vv]/100, 2, ',', '');
					if(isSet($x[$k]['subkat']) && is_array($x[$k]['subkat'])) {
						// Дополнительные характеристики
						foreach($x[$k]['subkat'] as $k_sk => $v_sk) {
							$x[$k]['subkat'][$k_sk][$vv]=number_format($v_sk[$vv]/100, 2, ',', '');
						}
					}
				}
				if(!empty($v['xx'])) {
	...
	function deleteItem($id){
		// Сначала находим у даляем страницу с подробным описанием данного элемента каталога
		global $engine;
		$res=$this->database->selectRow('select * from ?_katalog where `id`=?d', $id);
		$node = $engine->nodeScanId($res['the_href']);
		$engine->nodeDelete($node);
		// Потом удаляем сам элемент каталога и все дочерние элементы
		$this->database->query('delete from ?_katalog where `id`=?d OR `parent_id`=?d',$id,$id);
	}
	...
	function admin_katalog($cat=''){
		...
		//print_r(parent::admin_plugin(false));
		$return =
