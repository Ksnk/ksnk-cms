...
'.tpl::_a($par['head'],array('tpl_the','head')).'
<h2>Список категорий</h2>
<div>
<ul class="text">
'.tpl::_a($par['row'],array('tpl_the','row')).'
</ul></div>
...
<table  align="center" width="95%" border="0" cellpadding="0" cellspacing="0">
    ...
    <tr>
        ...
        <td><select name="select" value="'.(isset($par['id'])?$par['id']:'').'">
            <option value="root">-- корневая категория --</option>
            '.(isset($par['list_select'])?$par['list_select']:'').'
        </select></td>
        <td>
...
function head(&$par){		
		return tpl::_d($par['first'],'\\').'<a href="?id='.(isset($par['id'])?$par['id']:'').'">'.(isset($par['name'])?$par['name']:'').'['.(isset($par['id'])?$par['id']:'').']</a>';
}

function row(&$par){		
		return '<li style="padding:2pt;">
     <a href="#" id="id-'.(isset($par['id'])?$par['id']:'').'" onclick="Edit(this);return false;" class="draggable droppable">'.(isset($par['name'])?$par['name']:'').' ['.(isset($par['id'])?$par['id']:'').']</a>
     <span id="id+'.(isset($par['id'])?$par['id']:'').'" class="droppable" style="position:relative; top:7pt;color:blue;">&#8629;</span>
     '.(isset($par['ul'])?$par['ul']:'').'
</li>';
}}
?>