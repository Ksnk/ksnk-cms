...
(function($) {
  ...
  function disable(editor, disabled) {
    ...
    try {
      if ($.browser.msie) editor.$frame[0].contentWindow.document.body.contentEditable = !disabled;
      else editor.$frame[0].contentWindow.document.designMode = !disabled ? "on" : "off";
    }
  ...
  function execCommand(editor, command, value, useCSS, button) {
    ...
    if (!$.browser.msie) {
      if (useCSS === undefined || useCSS === null)
        useCSS = editor.options.useCSS;
      editor.$frame[0].contentWindow.document.execCommand("styleWithCSS", 0, useCSS.toString());
    }

    // Execute the command and check for error
    var success = true;
    if (command.toLowerCase() == "inserthtml" && $.browser.msie)
      editor.$frame[0].contentWindow.document.selection.createRange().pasteHTML(value);
    else {
      try { success = editor.$frame[0].contentWindow.document.execCommand(command, 0, value || null); }
  ...
  function selectedText(editor) {
    if ($.browser.msie) return editor.$frame[0].contentWindow.document.selection.createRange().text;
  ...
  function updateFrame(editor) {
	...
	editor.$frame.contents().find('body').html(editor.$area.val())
	editor.disable(false);
  }

  // ● updateTextArea - updates the textarea with the iframe contents
  function updateTextArea(editor) {
    var html = editor.$frame.contents().find('body').html(), $area = editor.$area;
    if (html != $area.val()) {
      $area.val(html);
