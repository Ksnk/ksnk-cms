...
class sysPar extends plugins {
	...
	function setUrl($arr){
		if ($this->url===FALSE) {
			...
			$this->root=preg_replace('~/index\.php~i','',$_SERVER['SCRIPT_NAME']);
		}
...
//				$this->db->query("COMMIT");
		...
		// элементы стоят далеко друг от друга
		$level=$row['level'];
		$left_key=$row['lid'];
		$right_key=$row['rid'];
		// изолируемсо
	...
	// блокируем - помечаем для переноса
		$test_cnt=2*$this->database->query(
			"update ?_tree set `ignored`=1 where lid >=$left_key AND rid <= $right_key"
		);
		if($test_cnt!=$child_cnt) echo('ахтунг!');
	// убираем элемент из дерева
		...
		if($parent_id<=0){
			...
			$parent=$parent_id;
			if(!$right_key_near) $right_key_near=$this->database->selectCell('select max(`rid`) from ?_tree')+1;
			$skew_level =$level_up-$level + 1 ;
			$skew_tree =$right_key-$left_key + 1;
			// бронируем место в начале списка
			$lid=$ins_row['lid'];
			$this->database->query(
				"UPDATE ?_tree SET rid = rid + $child_cnt,  lid = IF(lid > $lid, lid + $child_cnt, lid) WHERE `ignored`=0 and lid >= $lid "
			);
		// вставляемсо
		    $child_cnt=$ins_row['lid']-$row['lid']+1;
		    $this->database->query(
				"UPDATE ?_tree SET rid=rid+$child_cnt,  
				lid=lid+$child_cnt, level=level+$skew_level,`ignored`=0 WHERE `ignored`=1;"
			);
		} else {
	...
	function nodeAdd($parent_id,$key=array()) {
		...
		if($parent_id<0){
			...
			$keys=array(
				...
				'rid'=>$row['rid']+2,'level'=>$row['level'],'page'=>$key['id']);
			if(isset($key['name']))
				$keys['name']=$key['name'];
 ...
 */
	...
	function do_search(){
		...
		$this->go($this->getUrl(array('do'=>'search'))); //
	}
	...
	function do_qalist(){
		...
		return $this->export('qa','do_qalist');
	}
...
LIMIT 100;';
	...
	function do_add(){
		$this->sessionstart();
		$this->export('basket','addItem',pps($_GET['item']));
	...
	function curl($par='',$par2='',$par3=''){
		...
		if($par=='do'){
			unset($x['item'],$x['user'],$x['year'],$x['pg'],$x['url'],$x['ch'],$x['plugin'],$x['cat']);
		}
		if (empty($x)) return'?';
 ...
 */
	function go($where='',$path='') {
		...
		if(!empty($path)){	
			$par='http://'.$_SERVER["SERVER_NAME"].$path;
		} else
...
function convert_href($s){
	...
	if(!defined('IS_ADMIN') && (class_exists('altname'))) {
		$xfunc=create_function(
           ...
           '$x=altname::getrealaddr0($matches[2]);'.//$GLOBALS[\'altname\']->getrealaddr0($matches[2]);'.
           'if($x==$matches[2])
           		$x=preg_replace(
           		array(\'#\?do=(\w+)$#\',\'#\?do=menu&id=(\d+)$#\',\'#\?do=(\w+)&id=(\d+)$#\')
           		,array(\'/\\1\',\'/\\1\',\'/\\1/\\2\'),$x);'.
...
$url=preg_replace('#^(.*<%=$target_dir%>/index.php/|<%=$target_dir%>/)#','',$_SERVER['PHP_SELF']);
//printf('"%s" "%s"',$_SERVER['REQUEST_URI'],$url);
	DATABASE();
if(!empty($url)){
//	<% if (!empty($auto_plugins) && in_array('altname',$auto_plugins)){ %>
