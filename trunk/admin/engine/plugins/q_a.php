...
class qa extends ml_plugin {
	...
	function data($what,$from='',$perpage=''){
		if(defined('QA_WITH_THEME')){
			if(pps($_GET['url']))
				...
				$theme=pps($sm->v['descr'],$sm->v['name']);
			}		
	...
	function qa($parent){
		...
		'title'=>'Вопрос-Ответ'
		,'fields'=>array(
					array('тема','theme','menu','xoptions'),		
 ...
 * @return unknown
 */
	function do_qa($unAnsweredOnly=true,$tpl='qa_list',$theme='',$id_n=0){
		ml_plugin::setupmenu();
		...
		$pages='';
		if($id_n){
			$res=$this->database->select('select * from ?_qa where `id`=?d',$id_n);
		} else { 
...
/*		$locale = 'ru_RU.1251';
	...
	function do_qalist(){
		return $this->do_qa(true,'qa_list2',$theme='',pps($_GET['id']));
	}
...
class writeus extends ml_plugin {
	...
	function writeus(&$parent){
		...
		if($parent->is_ajax)
			$par["Тема"]=array('theme','hidden','value'=>pps($_GET['theme'],'qa'));
