...
order by `id` LIMIT 50;';
		...
		foreach($res as $v){
			...
			$result[]=array(
				'tag'=>'<a href="'.$this->parent->getRootUrl('newslist').'">Новости</a>',
				'text'=>'<a href="'.$this->parent->getRootUrl('news',$v['id']).'">'.$x.'</a>'
			);
		}
		//debug($result);
		return $result;
...
//*--*/ echo('/(?:\w+\W+){'.$this->get_limit(2).'}/');// LIMIT '.($page*$perpage).','.$perpage);
		...
		if($tpl=='news_b'){
			//debug($res);
			return $x[$tpl]=$this->parent->_tpl('tpl_news','_news_b',array('news'=>$res,'pages'=>$pages,'years'=>$years));
		}
		$news=array();
		if (!empty($res))
		foreach($res as $v) {
	...
	//debug(array(NEWS_TPL,''.$tpl));
		if($tpl=='news_b'){
			return $x[$tpl]=$this->parent->_tpl('tpl_news','_news_b',array('news'=>$news,'pages'=>$pages,'years'=>$years));
		} else
			return $x[$tpl]=smart_template(array(NEWS_TPL,''.$tpl),array('news'=>$news,'pages'=>$pages,'years'=>$years));
	}
	...
	//	debug('"'.$s.'"');
		...
		if(!empty($sm)){
			...
			$pic->v=array(
				'pic_comment'=>'',
				'item_url'=>$tpl=='news_b'?$this->parent->getRootUrl('newslist',$v['id']):'',
