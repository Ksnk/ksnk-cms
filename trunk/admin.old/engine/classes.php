...
class xTable extends xElement {
	...
	// row - header
		...
		for($i=1;$i<count($this->el);$i++){
			...
			if($this->v['type'] == type_KATALOG1 && !ereg('.+admin/index.jtpl', $_SERVER['SCRIPT_FILENAME']))
		...
		for($i=0;$i<count($this->el);$i++){
			...
			if (isset($v['text']))
			{
				...
				if($this->parent->v['type'] == type_KATALOG1 && $i==$this->parent->img_col && $x['td'] == 'td') {
					...
					if(ereg('.+admin/index.jtpl', $_SERVER['SCRIPT_FILENAME'])) {
						if ($x['pic_small'] == '')
				...
				if($this->parent->v['type'] == type_KATALOG1 && $i==$this->parent->osn_col && (!ereg('.+admin/index.jtpl', $_SERVER['SCRIPT_FILENAME']) && $x['td'] == 'td') && !empty($this->el[$this->parent->op_col]->v['text'])) {
					$x['text'] = '<a href="javascript:show_op('.$this->v['id'].');">'.$x['text'].'</a>';
				}
				if($this->parent->v['type'] == type_KATALOG1 && $i==$this->parent->op_col && !ereg('.+admin/index.jtpl', $_SERVER['SCRIPT_FILENAME'])) {
					$this->parent->v['opisanie'] = $x['text'];
					continue;
				}
				if($this->parent->v['type'] == type_KATALOG1 && $i==$this->parent->op_col && ereg('.+admin/index.jtpl', $_SERVER['SCRIPT_FILENAME']) && $x['td'] == 'td') {
					$x['class_r'] = "html_edit";
				}
