...
/**
 * реализация store для кук.
 * TODO: наваять еще такое-же для хранилища Хрома
 */
function store(_id,key,value,options){
	...
	if(typeof(value)!='undefined'){ // store value
		store[_id][key]=value;
		cookie(_id,$.param(store[_id]));
	} else {
		return store[_id][key]||'';
	}
}
...
;/****finish point js_admin_function *//*
*/
...
function order(e,s){
	var $v=$(e).parent().parent().find('.order:first');
	var x = $v.val().match(/\d+/);
	$v.val(s+(x && x[0]||1));
	need_Save(true);
}
...
function lookat(el){
	...
	if(!el.tagName || !el.tagName.toLowerCase) return 'oops!';
...
function action (act,first,second){
	switch (act){
	case 'insertElement':
		var x=lookat(first),y=lookat(second),z=false,m;
		if(m=y.match(/(\w+)_(\d+)/)){
			y=m[2];z=m[1];
		} else
			break;
		need_Save(true,function(){
			$.post('?do=ajax_insertElement','elem='+x.substr(3)+'&id=' +y,function(data){
				__goto();
			});
		});
		break;	
	case 'preview':
		window.open(window.location.toString().replace('/admin',''),'sample');
		break;
	case 'logout':
		$.get('?do=logout','',function(data){
			__goto();
		});
		break;
	case 'save': /* just submit a form */
		if(typeof(second)=='function'){
			$.post(document.location.toString(),'ajax=1&'+$(first.form).serialize(),second);
		} else
		if (first && first.form) first.form.submit();
		else {
			var f=$(this).parents('form');
			if(f)
				f.eq(0).submit();
			else
				alert('cannot save data by this button');
		}
		break;
	case 'newElement':
		var x=lookat(first),y=lookat(second),z=false,m;
		if(m=y.match(/(\w+)_(\d+)/)){
			...
			y=0;
		if(z=='newElem'){ // Вставить первый элемент в новую статью.
			need_Save(true,function(){
				$.post('?do=ajax_insertArticle','elem='+x.substr(3)+'&id=' +y,function(data){
					__goto();
				});
			});
		} else
		if (x!='katalog'){
			$(first).parents('form').first()
				.find('[name=new_item_type]').val(x.substr(3)).end()
				.find('[name=new_item_add]').removeAttr('disabled').val('ok').end()
				.find('[name=appendinto]').removeAttr('disabled').val(z).end()
				.find('[name=insert_after]').val(y).end()
				.submit();
		} else {
			alert('insert Element '+lookat(first));
		}
		break;
	case 'delete':	
		var p1=lookat(first),t;
		if (t=p1.match(/do=menu.*id=(\d+)/)){
			p1=t[1];
			if(confirm('Раздел будет удален. Вы уверены? ('+p1+')'))
				$.post('?do=ajax_delItem','x=menu&id=' +p1,function(data){
					__goto();
				});
		} else if(t=p1.match(/do=page.*id=(\d+)/)){
			p1=t[1];
			if (confirm('Элемент будет удален. Вы уверены?')) {
				$.post('?do=ajax_delItem', 'x=page&id=' + p1+p2, function(){
					__goto();
				});
			}
		} else if (t=p1.match(/\w+_(\d+)/)){
			win_confirm({
				txt:'Хотите удалить елемент?('+p1+')',
				yes:function(){
					$(first).parents('form').first()
					.find('input[name=del]').val(p1)
					.end()
					.submit();
				}
			});
/*			if(confirm('Хотите удалить елемент?('+p1+')')){
				$(first).parents('form').first()
					.find('input[name=del]').val(p1)
					.end()
					.submit();
			...
			alert('delete Element '+lookat(first));
		break;
	case 'move':	
		//alert('move Element '+lookat(first)+' before element '+lookat(second));
		$.post('?do=ajax_moveNodes','x='+lookat(first).substr(3)+'&y='+lookat(second).substr(3),function(){
			__goto();
		});
		break;
	default:
		alert(act+ (first && ('  '+lookat(first))||''));
	}
}

$(function(){
if($(document).preventSelection)$(document).preventSelection();
setTimeout(function(){$('select,input,textarea').placeholder();},10);

$('.state3,.stateX3').hover(
		function(){
			$(this).addClass('hover');
		}
		,function(){
...
new function(){
    var states={ // 0 - usual, 1 hover, 2 - clicked , 4 passive
    	delrec:{bg:['-227px -5px','-227px -29px'],w:16,h:16},
    	remrec:{bg:['-249px -5px','-249px -29px'],w:16,h:16},
        savebutton:{bg:['-0px -176px','-0px -116px','-0px -146px','-0px -86px'],w:120,h:22},
        button:{bg:['-0px -206px','-101px -206px','-201px -206px']
        		,col:['black','rgb(96,139,52)','black'],w:100,h:24},
    	preview:{bg:['-125px -86px','-125px -116px','-125px -146px','-125px -176px'],w:111,h:24},
    	undo:{bg:['-91px 0px','-91px -25px','-91px -50px'],w:32,h:25},
    	redo:{bg:['-131px 0px','-131px -25px','-131px -50px'],w:32,h:25},
    	help:{bg:['-198px -5px','-198px -30px','-198px -55px'],w:18,h:16},
    	logout:{bg:['-174px -5px','-174px -30px','-174px -55px'],w:18,h:16},
    	checkbox:{bg:['-249px -55px','-227px -55px','-293px -55px','-271px -55px'],w:16,h:15},
    	colorbox:{bg:['-249px -85px'],w:17,h:18},
    	fontbox:{bg:['-249px -112px'],w:17,h:18},
    	sizebox:{bg:['-249px -112px'],w:17,h:18},
    	selectbox:{bg:['-249px -112px'],w:17,h:18},
    	upload:{bg:['-275px -83px'],w:17,h:18},
    	_underline: {bg:['-305px -134px','-305px -168px'],w:32,h:32},
    	_bold:		{bg:['-272px -134px','-272px -168px'],w:32,h:32},
    	_italic:	{bg:['-239px -134px','-239px -168px'],w:32,h:32},
    	bgr_arrow:	{bg:['-340px 0px','-340px -65px','-340px -130px'],w:25,h:65},
       	brd_arrow:	{bg:['-365px 0px','-365px -65px','-365px -130px'],w:25,h:65},
       	arrowup:	{bg:['-313px -4px','-313px -22px','-313px -41px'],w:10,h:11},
       	arrowdn:	{bg:['-325px -4px','-325px -22px','-325px -41px'],w:10,h:11},
       	arrowlf:	{bg:['-272px -4px','-272px -22px','-272px -41px'],w:10,h:11},
       	arrowrt:	{bg:['-285px -4px','-285px -22px','-285px -41px'],w:10,h:11},
    	nothing:''
    };
	var aligns=
		[	'-4px -4px', // left
	...
	$.fn._4state=function(o){
		...
		if(states[o.name].w)
			this.css('width',states[o.name].w);
		if(states[o.name].h)
			this.css('height',states[o.name].h);
		return this.css({
			background: 'url(img/align.gif) no-repeat 0 0px',
			outline:'none',
		    border: 0,
		    cursor: 'pointer'
	    })
		.hover(
			 function(){if(!$(this).attr('disabled'))setState(this,o.name,1);}
			,function(){
   	...
   	function setAlign(e,n,hover){
   		n=Number(n)||0;
   		if(hover)
   			e.style.backgroundPosition=aligns[3+n];
   		else if(e.gray)
   			e.style.backgroundPosition=aligns[n+6];
   		else	
   			e.style.backgroundPosition=aligns[n];
   	}
 ...
 */ 
 ...
 var fonts=        // font names in the font popup
     "Arial,Arial Black,Comic Sans MS,Courier New,Narrow,Garamond," +
     "Georgia,Impact,Sans Serif,Serif,Tahoma,Trebuchet MS,Verdana"
	,sizes=        // sizes in the font size popup
     "8px,10px,11px,12px,14px,18px,20px,22px,24px"
	,colors=//E3E6EA
        "FFFFFF FFCCCC FFCC99 FFFF99 FFFFCC 99FF99 99FFFF CCFFFF CCCCFF FFCCFF " +
        "E3E6EA FF6666 FF9966 FFFF66 FFFF33 66FF99 33FFFF 66FFFF 9999FF FF99FF " +
        "C0C0C0 FF0000 FF9900 FFCC66 FFFF00 33FF33 66CCCC 33CCFF 6666CC CC66CC " +
        "999999 CC0000 FF6600 FFCC33 FFCC00 33CC00 00CCCC 3366FF 6633FF CC33CC " +
        "576573 990000 CC6600 CC9933 999900 009900 339999 3333FF 6600CC 993399 " +
        "333333 660000 993300 996633 666600 006600 336666 000099 333399 663366 " +
        "000000 330000 663300 663333 333300 003300 003333 000066 330099 330033";
 //colorPopup
    var $popup=$('#colorPopup');  
    $.each(colors.split(" "), function(idx, color) {
        $('<div/>').appendTo($popup)
...
//fontPopup
    var $popup=$('#fontPopup');  
    $.each(fonts.split(","), function(idx, font) {
        $('<div>').appendTo($popup)
...
//sizePopup
    var $popup=$('#sizePopup');  
    $.each(sizes.split(","), function(idx, size) {
        $('<div>').appendTo($popup)
          .html("<font style='font-size:" + size + ";'>" + size + "</font>");
      });

    $popup.appendTo(document.body).hide();
//  
var popups={font:1,color:1,size:1};
for(var a in popups)
$('.'+a+'box').wrapdiv() // трансформируемся!
.after('<input type="text" class="overbutton" >')
.parent().find('.overbutton').css({
		background: 'url(img/align.gif) no-repeat '+states[a+'box'].bg[0],
	    border: 0,
	    outline:'none',
	    cursor: 'pointer',
	    top:0,
	    width:states[a+'box'].w,
	    height:states[a+'box'].h
    }).data('psuf',a)
.click(function(){
	var par = $(this).parent().offset(),a=$(this).data('psuf');
	par['top']+=$(this).parent().height();
	
	$('#'+a+'Popup').css(par).show().data('input',$(this).parent().find('.'+a+'box'));
	$(document.body).one('mousedown',function(e){
        var target = e.target;
        if($(target).add(target.parentNode).is('#'+a+'Popup div')){
            if(a=='color')
	    		value = hex(target.style.backgroundColor);
            else if (a=='font')
            	value = hex(target.style.fontFamily);
            else if (a=='size')
                value=$(target).text();               
	    	$('#'+a+'Popup').data('input').val(value).trigger('change');
        }
		$('#'+a+'Popup').hide();
	});
	return false;
...
// fontbox
/*
$('.fontbox') // трансформируемся!
.wrap('<div class="wrapper"/>')
.after('<input type="text" class="overbutton" >')
.parent().find('.overbutton').css({
		background: 'url(img/align.gif) no-repeat '+states['fontbox'].bg[0],
	    border: 0,
	    cursor: 'pointer',
	    outline:'none',
	    top:0,
	    width:states['fontbox'].w,
	    height:states['fontbox'].h
    })
.click(function(){
	var par = $(this).parent().offset();
	par['top']+=$(this).parent().height();
	$('#fontPopup').css(par).show().data('input',$(this).parent().find('.fontbox'));
	$(document.body).one('mousedown',function(e){
        var target = e.target;
        if($(target).is('#fontPopup div')){
        	value = hex(target.style.fontFamily);
	    	$('#fontPopup').data('input').val(value);
        }
		$('#fontPopup').hide();
	});
	return false;
...
$('.upload').each(function(){ // трансформируемся!
	var alt=$(this).attr('name').match(/(big|small)/);
	if(alt) alt=alt[1];
	else return ;
	$(this).wrapdiv()
	.after('<input type="text" class="overbutton" >')
	.parent().find('.overbutton').css({
		background: 'url(img/align.gif) no-repeat '+states['upload'].bg[0],
	    border: 0,
	    cursor: 'pointer',
	    outline:'none',
	    top:0,right:'2px',
	    width:states['upload'].w,
	    height:states['upload'].h
    }).attr('alt',alt);
});
/** *************************************************************
 *   logout functional
 */   	
$('#logout')._4state('logout')
.click(function(){
	action('logout');
});
 ...
 *   bold functional
 */   	
 //var popups={_bold:1,_underline:1,_italic:1}
 for(var a in {_bold:1,_underline:1,_italic:1}){
	 $('.'+a)
.css({cursor:'pointer',outline:'none',
fontSize:'0px',background:'url(img/align.gif) no-repeat'
	,color:'black',border:0,height:states[a].h,width:states[a].w}).data('sss',a)
.each(function(){
	var self=this,a=$(this).data('sss');
	$(this).click(function(){
		var v = Number(this.value)||0;
		this.value=(v=(v==0?1:0));
		if ((this.value==1) && $(this).hasClass('nocenter'))
			this.value=(v==2?0:v+1);
   		setState(this,a,this.value);
   		$(this).trigger('change');
   		$('#help').focus();
   		need_Save();
	})
	.hover(
		 function(){var a=$(this).data('sss');this.hover=true;setState(this,a,this.value);}
		,function(){var a=$(this).data('sss');this.hover=false;setState(this,a,this.value);})
	.trigger('mouseout');
});
 }
//$('._underline')._4state('_underline');
//$('._italic')._4state('_italic');

/** *************************************************************
 *   undo functional
 */   	
$('#undo')._4state('undo')
.click(function(){action('undo');});
/** *************************************************************
 *   redo functional
 */   	
$('#redo')._4state('redo')
.click(function(){action('redo');});
 	
   	
/** *************************************************************
 *   updn arrow functional
 */   	
function order(el,sub){
	// ищем рядомстоящий пункт input и изменяем его значение
	var input=$(el).parent().find('input:text'),val=parseInt(input.val())||0;
	input.val(val+sub).trigger('change');
}
 
$('input.arrowup')._4state('arrowup').click(function(){order(this,-1)});
$('input.arrowdn')._4state('arrowdn').click(function(){order(this,1)});
$('input.arrowrt')._4state('arrowrt');
$('input.arrowlf')._4state('arrowlf');

/** *************************************************************
 *   savebutton functional
 */   	
var $must_save=false;
$('input.savebutton')._4state('savebutton')
.click(function(){
	if($must_save)
	{
		$must_save=false;
...
$('input.button')._4state('button')
/**
 * севис сохранения данных
 * -- false - безусловно сбросить флаг сохранения
 * -- true - сделать submit, если указан callback - сохранение с помощью ajax'а 
 *    и вызов callback после успеха. 
 * -- без параметров - поднять флаг сохранения, нужно переспросить при выходе
 */
...
window.need_Save= function(par,callback){
	if(!par && par===false)
		$must_save=false;
	else if(par===true){
		$must_save=false;
		action('save',$('.savebutton')[0],callback);
	} else	
		$must_save=true;
	if($must_save){	
		setState(
			$('.savebutton').removeAttr('disabled')[0]
			,'savebutton',0
		);
	}
	else {
		setState(
			$('.savebutton').attr('disabled','disabled')[0]
			,'savebutton',3
		);
	}	
}; 
 ...
 ** *************************************************************  
 */   	
$('.delrec')._4state('delrec')
.click(function(){
	action('delete',$(this).parents('[id!=]')[0]);
});	
$('.remrec')._4state('remrec')

 /** *************************************************************
 *   checkbox button functional
 */   	
$('.checkbox')
.css({cursor:'pointer',outline:'none',
fontSize:'0px',background:'url(img/align.gif) no-repeat',color:'white',border:0})
.each(function(){
	var  self=this,_def=$(this).attr('placeholder')
		,states=[0,1],values=['0','1']; // on off
	if (!!_def) {values=['0','1',''];states=[0,1,_def=='1'?3:2];}
	function _get(){
		if(self.value===values[0]) return 0;
		if(self.value===values[1]) return 1;
		return 2;
	}
	function _set(state){
		if(state>=values.length) state=0;
		if(self.value!="" && self.value!=values[state]) 
			need_Save();
		setState(self,'checkbox',states[state]);
		if(self.value!="" && self.value!=values[state]) {
			self.value=values[state];
   			$(self).trigger('change');
		} else 
			self.value=values[state];
	}
	...
	.hover(
		 function(){this.hover=true;_set(_get());}
		,function(){this.hover=false;_set(_get());}
	)
	.trigger('mouseout');
});
 /** *************************************************************
 ...
 */   	
$('.selectbox')
.parent().find('.options').hide().end().end()
.wrapdiv()
.after('<input type="text" class="overbutton" >')
.parent().find('.overbutton').css({
		background: 'url(img/align.gif) no-repeat '+states['selectbox'].bg[0],
	    border: 0,
	    cursor: 'pointer',
	    outline:'none',
	    top:0,
	    width:states['selectbox'].w,
	    height:states['selectbox'].h
    })
/*.click(function(){
	var par = $(this).parent().offset();
	par['top']+=$(this).parent().height();
	
	$(this).parent().parent().find('.options').css(par).show().data('input',$(this).parent().find('.selectbox'));
	$(document.body).one('mousedown',function(e){
        var target = e.target;
        if($(target).add(target.parentNode).is('#'+a+'Popup div')){
            if(a=='color')
...
$('.align').each(function(){
	...
	.clone().removeAttr('id')
	.each(function(){
		if($(self).parent().hasClass('bgray')) this.gray=true;
	})
	.appendTo($(this).parent())
	.click(function(){
		var v = Number(self.value)||0;
		self.value=(v=(v==2?0:v+1));
		if ((self.value==1) && $(this).hasClass('nocenter'))
			self.value=(v==2?0:v+1);
   		setAlign(this,self.value,this.hover);
   		$(self).trigger('change');
   		need_Save();
	})
	.hover(
		 function(){this.hover=true;setAlign(this,self.value,true);}
		,function(){this.hover=false;setAlign(this,self.value);})
	.trigger('mouseout');
});
...
window.win_dialog=function (par){
	var w,d=$( par.dialog );
	if(par.txt) d.attr('title',par.txt);
	if(!(w=d.css('width'))) w='auto';
	d.dialog({
		//height: 160,
		width: w,
		modal: true,
...
/**
 * инициализация глобального грузчика
 */	
	var locked=false,swfu_opts = {
		upload_url : "../?ajax=1&do=file_uploader"+'&PHPSESSID='+cookie('PHPSESSID'),
		...
		upload_complete_handler : function (file, errorCode, message){
		alert(['complete',file, errorCode, message]);	
	},
	upload_complete_handler: function (file, data, message){
		this.startUpload();
	},
	upload_success_handler : function (file, data, message){
		// вариант - все в одном флаконе
		$('<span>').appendTo('#debug').text(data);
		...
		var _elem = $($('#swf_uploader').data('data'));

		if(!_elem.is('.fotopane')){
			var elem=_elem;
			if(elem.has('img').length==0){
				elem=_elem.parents('[id!=]').first();
			} else if(elem.has('input').length==0){
				elem=elem.parent();
			}
			...
			for(var a in data.result){
				if(a=='small'){
					$(elem)
						.find('input[name*=_small_]').val(data.result[a]).end()
						.find('img').attr('src',data.result[a]);
					need_Save();
				} else
				if(a=='big'){
					$(elem)
						.find('input[name*=_big_]').val(data.result[a]);
					need_Save();
				}
			}
		}
		if (this.total_queued-1==file.index){
			data.last=true;	
			locked=false;	
		}
		$(_elem).trigger('imgchanged',data);
	},
		upload_error_handler : function (file, errorCode, message){
			alert(['error',file, errorCode, message]);	
		},
...
/*		button_action : op.multiple ? SWFUpload.BUTTON_ACTION.SELECT_FILES : SWFUpload.BUTTON_ACTION.SELECT_FILE,
		...
		post_params: plugin_opts.post_params*/
		file_size_limit : "1 mb"
		,button_window_mode: "transparent"
	};	
/**
 * грузчик картинок посредством SWFUpload
 *
 * варианты грузки
 * -- один 
 */	
// установка попапа для установки 
 
// грузчик одной картинки
$('.upload+.overbutton, .picture_upload').hover(function(){
	if(!locked && !$(this).hasClass('swf_present')){
		var par = $(this).offset();
		par.width=$(this).width();
		par.height=$(this).height();
		$(this).addClass('swf_present');
		var swf=$('#swf_uploader')
		.attr('title',$(this).attr('title'))
		.css(par).show().data('data',this).data('swf');
		// установить тип файла 
		// swf.setFileTypes('рисунки',"*.bmp");
		// установить постпроцесс и размер картинки
		// swf.addPostParam('min','120x300');

		//swf.removePostParam('xaction');
		
	}
}, function (){
	if(!locked && $(this).hasClass('swf_present')){
		$(this).removeClass('swf_present');
	//	$('#swf_uploader').hide();
	}
})

$('<div id="swf_uploader"><div id="swf_container"/></div>').hide()
	.css({border:'1px solid gray',position:'absolute',zIndex:1005,overflow:'hidden'}).appendTo(document.body);
var swf = new SWFUpload($.extend(swfu_opts,{
	button_width : 100,
	button_height : 100,
	button_placeholder_id:'swf_container',
	button_action: SWFUpload.BUTTON_ACTION.SELECT_FILE,
	file_dialog_complete_handler:function (files_selected){
		this.total_queued=files_selected;
		if(!files_selected) locked=false;	
		if(files_selected){
			$data=$($('#swf_uploader').data('data'));
			var elem=$data.parents('[id!=]').first().attr('id');
			elem=elem && elem.match(/\w+_(\d+)/);
			if(elem)
				this.addPostParam('use'	,elem[1]);
			else
				this.removePostParam('use');
			this.addPostParam('xaction'
				,$data.attr('alt')||'both');
			this.startUpload();
		}
	}
...
$('#swf_uploader').data('swf',swf).mouseleave(function(){
	if(!locked) $(this).hide();
});
...
$('.text_edit').click(function(e){
	...
	if (!$(this).data('input')) {
		...
		$(this).html('').data('input',
			$('#textcntr_tpl').clone().removeAttr('id')
			.attr('name',this.id)
			.val(content.replace('&amp;','&').replace(/^\s+|\s+$/,''))
			.appendTo(this)
			.bind('blur',function(){
				var parent=$(this).parent();
				parent.data('span').html(
					$(this).hide().val()
				)
				.show();//.replace('>','&gt;').replace('<','&lt;'); // XXX: порнушко!!!
				setTimeout(function(){
					parent.data('span').html(
						parent.data('input').val())
				},300)
		...
		$(this).data('span',
			$('<div>').html(content)
			.appendTo(this).hide().css({width:$(this).width(),height:$(this).height()})
			.click(function(){
				$(this).hide().parent().data('input').show().focus();
			})
		);				
	};
});
...
function do_edit(ctrl){
	$("#dialog-modal").data('ctrl',ctrl);
	$('#EditorContent').val($(ctrl).val());
	ctrl=null; 
	var editor;
	if(!(editor=$("#dialog-modal").dialog('open').data('editor'))){
		var dialog=$("#dialog-modal").dialog({
			...
			title:'Редактор',
			resizable:false
		}).data('editor',$('#EditorContent').cleditor({width:760,height:430}))
		.parent() .find('.ui-dialog-titlebar-close').remove().end()
			.find('.ui-dialog-titlebar')
				.append('<input type="button" class="edcancelbuton" value=""/>')
				.append('<input type="button" class="edokbuton" value=""/>')
				.find('input').click(function(event) {
					if($(event.target).hasClass('edokbuton')){
						var editor= $("#dialog-modal").data('editor');
						editor[0].updateTextArea().hidePopups();
						$($("#dialog-modal").data('ctrl'))
							.val($('#EditorContent').val());	
						need_Save(true);
					}
					$("#dialog-modal").data( 'dialog' ).close(event);
					return false;
				})
				
		;
	} else {
		editor[0].updateFrame().focus();
	}
}

$('.html_edit').click(function(){
	if(!($(this).data('input'))){
		var self=this;
		$(this).data('input',
			$('#htmlcntr_tpl').clone().removeAttr('id')
				.attr('name',this.id).val($(this).html()).appendTo($(this))
		);
		$.get('?do=ajax_get_contents&var=' + this.id + '&id=' + $(this).parents('[id!=]')[0].id,
		function(data){
			data= (new Function("return " + data))(); 
			data.data = data.data.substr(1);
			do_edit($(self).data('input').val(data.data));
		})
	} else
		do_edit($(this).data('input'));			
});
//
/**
...
$('#katalog .stateX3').draggable({ appendTo: 'body', opacity: 0.7, helper: 'clone'});
$('.insertable').droppable({
accept:'.stateX3',
hoverClass:'drophover',drop: function(event, ui) {
	action('insertElement',ui.draggable,this);
}});
$('.droppable').droppable({
accept:function(el){
	if($(this).hasClass('expanded')) return false;
	if($(el).is('.id_picture,.stateX3'))
		return true;
},
hoverClass:'drophover',drop: function(event, ui) {
	if(ui.draggable.hasClass('id_picture')){
		action('move',ui.draggable,this);
	} else {
		action('newElement',ui.draggable,this);
	}
}});
$('.hmenu a').addClass('xxmenu');

$('.id_picture').draggable({ appendTo: 'body', helper: 'clone'});

$('#basket').droppable({accept: '.id_picture,.xxmenu',hoverClass:'drophover',drop: function(event, ui) {
     action('delete',ui.draggable);
}});
$('.hmenu a').draggable({ appendTo: 'body', opacity: 0.7, helper: 'clone'});

$('.hmenu a').droppable({accept: '.xxmenu',hoverClass:'drophover',drop: function(event, ui) {
     action('move',ui.draggable,this);
}});

$('#katalog a').click(function (){ action('newElement',this); return false;})
$('<div class="arrow"></div>').appendTo($('.graypane .dd_graypane').parent())
.click(function(){
	var $parent=$(this).parent();
...
$('.greenpane .datapane').parent().addClass('collapsed');
$('.greenpane .arrow').each(function(){
	if($(this).parent().find('.datapane').length>0){
		$(this).click(function(){
			var $parent=$(this).parent();
			if($parent.hasClass('expanded')){
				$parent.removeClass('expanded').find('.datapane').first().hide();
				store('openclose',$parent.attr('id'),0)	;
				$parent.find('.panebutton').removeClass('expanded');
			} else {
				var x=$parent.find('.panebutton');
				var pane=1;
				if(x.length){
					pane=x.first().trigger('click').data('pane');
				} else {
					$parent.addClass('expanded').find('.datapane').first().show();
				}
				store('openclose',$parent.attr('id'),pane)	;
			}
		});
	} else if($(this).is('.greenpane .greenpane .arrow')){
		$(this).css('background-position','-365px -130px');
	}else {
		$(this).css('background-position','-340px -130px');
	}
});
$('.greenpane td').click(function(e){
	if(this!=e.target) return;
	store('openclose',this,0)	;
	$(this).parents('table:first')
		.find('.panebutton').removeClass('expanded').end()
		.siblings('.datapane:first').hide()
		.parents('.greenpane:first').removeClass('expanded');	
});
$('.linkpane').data('pane','linkpane');
$('.commentpane').data('pane','commentpane');
$('.fotopane').data('pane','fotopane');
$('.haractpane').data('pane','haractpane');
$('.tablepane').data('pane','tablepane');
$('.notepane').data('pane','notepane');
$('.panebutton').click(function(e){
	var pane=$(this).data('pane'),number=$(this).attr('data-number')||'first';
	store('openclose',this,pane)	;
	if($(this).is('.linkpane.expanded'))
		action('insertElement',13,this);
	$(this).addClass('expanded').parents('table:first')
		.find('.panebutton.expanded').not(this).removeClass('expanded').end().end()
		.siblings('.datapane:first').show()
//	.parents('.greenpane:first').find('.datapane:first').show().end()
//		
		.parents('.greenpane:first').addClass('expanded').end()
		.children('div').not('.'+pane+'_pane:'+number).hide().end().end()
		.children('.'+pane+'_pane:'+number).show().end()
		.children('.fotopane.expanded').trigger('mouseover');
	if(!$(this).is('.fotopane'))
		$('#swf_uploader').hide();
	else {
...
$('.fotopane').hover(function(){
	if(!$(this).hasClass('swf_present') && $(this).hasClass('expanded')){
		var par = $(this).offset();
		par.width=$(this).width();
		par.height=$(this).height();
		$(this).addClass('swf_present');
		var swf=$('#swf_uploader').css(par).show().data('data',this).data('swf');
		swf.setButtonAction(SWFUpload.BUTTON_ACTION.SELECT_FILES );
		// установить тип файла 
		//swf.setFileTypes('рисунки',"*.bmp");
		// установить постпроцесс и размер картинки
		swf.addPostParam('min','120x300');
		swf.addPostParam('xaction','both');
	}
}, function (){
	if($(this).hasClass('swf_present')){
		$(this).removeClass('swf_present');
		//$('#swf_uploader').hide();
	}
}).bind('imgchanged',function(e,data){
	var parent=$(this).parents('[id!=]').first();

	if (!parent.length) return alert('oops!');
	var last = data.last||false;
	$.post('?do=ajax_gallery_img&id='+parent.attr('id'),
...
})/****finish point js_admin *//*
 */
	
	var openclose=store('openclose');
	for(a in openclose){
		if(openclose[a]!=0){
			if(openclose[a]==1)
				$('#'+a).find('.arrow:first').trigger('click');
			else
				$('#'+a).find('.'+openclose[a]+':first').trigger('click');
		}
	}
	...
	function justajax(e,href,repl){
		...
		if(form){
			...
			ajaxit=function($repl){
				if($repl) href=href.replace('~~',decodeURIComponent($repl));
				if(href){
					$.get(href.replace('ajax=1','').replace(/(do=\w*)/,'$1&ajax=1')
						,compl);
				}
			};
		}
