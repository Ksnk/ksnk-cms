...
function edit_table(&$par){		
		return '<table class="align_'.tpl::_d($par['align'],'center').'" style="border-top:3px solid #e3e6ea; border-bottom:3px solid #e3e6ea; border-left:none; border-right:none; margin-top:20px; margin-bottom:20px;">
...
function katalog(&$par){		
		return '<table style="border-spacing:0;width: 100%; margin: 0px 0 10px 0;">
...
'.tpl::_a($par['gallery'],array('tpl_elements','katalog_gallery')).'	
    '.tpl::_a($par['galleryX'],array('tpl_elements','katalog_galleryX')).'
</table>';
}
...
function katalog_textpic(&$par){		
		return '<tr>
			...
			<div style="padding-right:20px;">'.(isset($par['the_text'])?$par['the_text']:'').'</div>
...
function katalog_gallery_row_pict(&$par){		
		return '<td  style="vertical-align:bottom;padding:0 25px 5px 0;">
				...
				'.tpl::_ax(tpl::_export('','tpl','admin','imgpre'),array('tpl_elements','katalog_gallery_row_pict___tpl_admin_imgpre')).'<table>
                						<tr><td class="picture_box"><a rel="g'.(isset($par['pid'])?$par['pid']:'').'" href="'.tpl::_d($par['xhref'],'#').'" '.tpl::_format(pps($par['comment']),' title="%s"').'><img src="'.tpl::_ax(tpl::_export('','index'),array('tpl_elements','katalog_gallery_row_pict___index')).'/'.(isset($par['small_pict'])?$par['small_pict']:'').'" alt="'.tpl::_html(pps($par['comment'])).'" '.tpl::_brd(pps($par['border'])).' class="text_picture"></a></td></tr>
                                    </table>
				</td>';
}
...
$(function(){
    ...
    var div = $(\'div.sc_menu'.(isset($par['pid'])?$par['pid']:'').'\'),
                 ul = $(\'ul.sc_menu'.(isset($par['pid'])?$par['pid']:'').'\'),
    ...
    div.mousemove(function(e){
      ...
      div.scrollLeft(left);
			...
			<div id="sc_menu_id" class="sc_menu'.(isset($par['pid'])?$par['pid']:'').'" style="'.(isset($par['rp_sheight'])?$par['rp_sheight']:'').' '.(isset($par['float_p'])?$par['float_p']:'').'">
			<ul class="sc_menu'.(isset($par['pid'])?$par['pid']:'').'" style="'.(isset($par['rp_sheight'])?$par['rp_sheight']:'').'">
		'.tpl::_a($par['row'],array('tpl_elements','katalog_galleryX_row')).'
        		</div>
			</ul>
        </div> 
...
function katalog_galleryX_row(&$par){		
		return tpl::_a($par['pict'],array('tpl_elements','katalog_galleryX_row_pict'));
}

function katalog_galleryX_row_pict(&$par){		
		return '<li>'.tpl::_ax(tpl::_export('','tpl','admin','imgpre'),array('tpl_elements','katalog_galleryX_row_pict___tpl_admin_imgpre')).'<a rel="g'.(isset($par['gid'])?$par['gid']:'').'" href="'.tpl::_d($par['xhref'],'#').'" '.tpl::_format(pps($par['comment']),' title="%s"').'>
 				     <div><img src="'.tpl::_ax(tpl::_export('','index'),array('tpl_elements','katalog_galleryX_row_pict___index')).'/'.(isset($par['small_pict'])?$par['small_pict']:'').'" alt="'.tpl::_html(pps($par['comment'])).'"></div>'.tpl::_ax(tpl::_export('','tpl','admin','imgpost'),array('tpl_elements','katalog_galleryX_row_pict___tpl_admin_imgpost')).'
					 <span style="'.(isset($par['swidth_b'])?$par['swidth_b']:'').'">'.(isset($par['comment'])?$par['comment']:'').'</span>
			    </a></li>';
}
...
function nfirst(&$par){		
		return '<div class="nfirst1">
<a href="'.tpl::_ax(tpl::_export('','index'),array('tpl_elements','nfirst___index')).'/">Главная</a>&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;
'.tpl::_a($par['list'],array('tpl_elements','nfirst_list')).'
</div>';
}

function nfirst_list(&$par){		
		return '<a href="'.tpl::_ax(tpl::_export('','curl','do','id'),array('tpl_elements','nfirst_list___curl_do_id_topic')).'do=menu&id='.(isset($par['id'])?$par['id']:'').'">'.(isset($par['name'])?$par['name']:'').'</a>&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;';
}

function nfirst2(&$par){		
		return '<div class="nfirst2">
<h1>'.(isset($par['sub'])?$par['sub']:'').'</h1>
</div>';
}
...
function basket_top(&$par){		
		return '<nobr><span class="red"><b>'.tpl::_d($par['pos'],'нет').'</b></span> '.tpl::_d($par['posx'],'товаров.').'<br>
Сумма <span class="red"><b>'.(isset($par['cost'])?$par['cost']:'').'</b></span> руб.</nobr>';
}
...
function table_parts_td_input(&$par){		
		return '<td class="align_right" style="vertical-align:middle; text-align:left;">
		<input name="item_'.(isset($par['id'])?$par['id']:'').'" value="'.(isset($par['value'])?$par['value']:'').'" style="border:1px solid #657382; background-color:#424d5b; width:60px; color:#FFFFFF" type="text"></td>';
}
...
function catalogue(&$par){		
	...
	<tr><td class="align_left" style="padding-left:60px; padding-bottom:15px">
		...
		<td class="white size11" style="vertical-align:middle;"><input type="submit"
			style="border:0;background-color:#FFFFFF"
			class="basket_link"
			value="'.tpl::_d($par['button_val'],'Добавить в корзину').'">
        </td></tr></table>
    ...
    <table style="width:100%; border-top:3px solid #e3e6ea; border-bottom:3px solid #e3e6ea; border-left:none; border-right:none; margin-top:0px; margin-bottom:8px;">
		<tr><td style="padding-top:1px; padding-bottom:1px; padding-left:-1px; overflow:hidden;">
		    <table class="long size12 table tablex" style="margin-left:-1px;">
				<tr class="size11">'.(isset($par['headers'])?$par['headers']:'').'</tr>
...
function catalogue__table_data(&$par){		
		return '<tr id="xx_'.(isset($par['id'])?$par['id']:'').'" class="'.tpl::_b($par['even'],'even','odd').'">'.(isset($par['data'])?$par['data']:'').'</tr>';
}

function catalogue__table_subtitle(&$par){		
		return '<tr><td class="menu size12" style="text-align:right">
			<div class="align_right" style="padding-right:10%; float:right">'.(isset($par['title'])?$par['title']:'').', руб:&nbsp;&nbsp;&nbsp;'.(isset($par['value'])?$par['value']:'').'</div>
	</td></tr>';
}
...
function xcatalogue__table_data(&$par){		
		return '<tr>
				<td>
					<table class="box">
                    	<tr><td>&nbsp;</td><td width="100%" class="name">'.(isset($par['name'])?$par['name']:'').'</td></tr>
                        <tr style="height:18px"><td>&nbsp;</td><td>&nbsp;</td></tr>
						<tr class="ajaxform">
							<td class="gallery">
								<input type="hidden" name="id_'.(isset($par['id'])?$par['id']:'').'" value="'.(isset($par['id'])?$par['id']:'').'">
									'.tpl::_ax(tpl::_export('','tpl','admin','imgpre'),array('tpl_elements','xcatalogue__table_data___tpl_admin_imgpre')).'<div style="margin-right:50px;"><a rel="g'.(isset($par['id'])?$par['id']:'').'" href="'.tpl::_d($par['xhref'],'#').'" '.tpl::_format(pps($par['comment']),' title="%s"').'>
									<img src="'.tpl::_ax(tpl::_export('','index'),array('tpl_elements','xcatalogue__table_data___index')).'/'.tpl::_d($par['small_pict'],'img/empty_pic.gif').'" alt="'.tpl::_html(pps($par['comment'])).'" class="pic"></a></div>'.tpl::_ax(tpl::_export('','tpl','admin','imgpost'),array('tpl_elements','xcatalogue__table_data___tpl_admin_imgpost')).'
							</td>
							<td><table><tr><td class="descr">'.(isset($par['descr'])?$par['descr']:'').'</td></tr></table>
                            
                            <table style="margin-top:15px">
								<tr>
									<td><a href="javascript:dop_har('.(isset($par['id'])?$par['id']:'').');" '.tpl::_b($par['subkat_s'],'class="strelka"','').'>'.tpl::_b($par['subkat_s'],'Купить...','').'</a></td>
									<td><a href="'.(isset($par['url'])?$par['url']:'').'/'.(isset($par['the_href'])?$par['the_href']:'').'" '.tpl::_b($par['show_podr'],'class="strelka"','').'>'.tpl::_b($par['show_podr'],'Описание...','').'</a></td>
								</tr>
							</table>
                            </td>
						</tr>
...
<tr><td style="padding-top:20px; padding-bottom:30px;">
	<table class="align_'.tpl::_d($par['align'],'center').' dop_har dop_har_'.(isset($par['id'])?$par['id']:'').'" style="width:90%; border-top:3px solid #e3e6ea; border-bottom:3px solid #e3e6ea; border-left:none; border-right:none;">
		<tr><td style="padding-top:1px; padding-bottom:1px;">
...
function xcatalogue__table_data_s_data(&$par){		
		return '<tr><td>
	<table class="align_'.tpl::_d($par['align'],'center').'" style="width:90%; border-top:3px solid #e3e6ea; border-bottom:3px solid #e3e6ea; border-left:none; border-right:none; margin-top:20px; margin-bottom:30px;">
		<tr><td style="padding-top:1px; padding-bottom:1px">
			<table class="table tablex" style="width:100%">
				...
				<tr class="ajaxform">
					<td class="first_td" style="padding-right:20px">'.(isset($par['name'])?$par['name']:'').'</td>
					<td style="color:#b41414; font-size:14px; font-weight:600; padding-right:30px; background:url('.tpl::_ax(tpl::_export('','index'),array('tpl_elements','xcatalogue__table_data_s_data___index')).'/img/rubles.gif) 100% 51% no-repeat">'.(isset($par['cost'])?$par['cost']:'').'</td>
					<td>'.(isset($par['ostatok'])?$par['ostatok']:'').'</td>
					<td><input type="hidden" name="id_'.(isset($par['id'])?$par['id']:'').'" value="'.(isset($par['id'])?$par['id']:'').'"><input style="border:1px solid #e0e0e0" type="text" style="width:45px;" name="item_'.(isset($par['id'])?$par['id']:'').'" value="'.(isset($par['value'])?$par['value']:'').'"></td>
					<td style="padding-right:35px"><div><a href=\'#\' class="submit blue"><img src="'.tpl::_ax(tpl::_export('','index'),array('tpl_elements','xcatalogue__table_data_s_data___index')).'/img/basket.gif"></a></div></td>
				</tr>
...
function xcatalogue__table_data_subkat(&$par){		
		return '<tr class="ajaxform dop_har dop_har_'.(isset($par['parent_id'])?$par['parent_id']:'').'">
					<td class="first_td">'.(isset($par['name'])?$par['name']:'').'</td>
					<td style="color:#b41414; font-size:14px; font-weight:600; padding-right:20px; background:url('.tpl::_ax(tpl::_export('','index'),array('tpl_elements','xcatalogue__table_data_subkat___index')).'/img/rubles.gif) 100% 51% no-repeat">'.(isset($par['cost'])?$par['cost']:'').'</td>
					<td>'.(isset($par['ostatok'])?$par['ostatok']:'').'</td>	
					<td><input type="hidden" name="id_'.(isset($par['id'])?$par['id']:'').'" value="'.(isset($par['id'])?$par['id']:'').'"><input style="border:1px solid #e0e0e0" type="text" style="width:45px;" name="item_'.(isset($par['id'])?$par['id']:'').'" value="'.(isset($par['value'])?$par['value']:'').'"></td>
					<td style="padding-right:35px"><div><a href=\'#\' class="submit blue"><img src="'.tpl::_ax(tpl::_export('','index'),array('tpl_elements','xcatalogue__table_data_subkat___index')).'/img/basket.gif"></a></div></td>
				</tr>';
}
...
<div>'.tpl::_ax(tpl::_export('','tpl','admin','imgpre'),array('tpl_elements','xxcatalogue__table_data___tpl_admin_imgpre')).'<a rel="g'.(isset($par['pid'])?$par['pid']:'').'" href="'.tpl::_d($par['xhref'],'#').'" '.tpl::_format(pps($par['comment']),' title="%s"').'>
					<img src="'.tpl::_d($par['small_pict'],'\'www.xln.su/test/ezavod/img/empty_pic.\'').'" alt="'.tpl::_html(pps($par['comment'])).'"></a>'.tpl::_ax(tpl::_export('','tpl','admin','imgpost'),array('tpl_elements','xxcatalogue__table_data___tpl_admin_imgpost')).'</div>
</td></tr><tr>
...
function katalog_back(&$par){		
		return '<div class="name">'.(isset($par['name'])?$par['name']:'').'</div>
<div style="margin-top:30px; margin-bottom:20px; float:none; "><a href="'.(isset($par['addr'])?$par['addr']:'').'">&lt;&lt; Назад</a></div>';
}

function katalog_back_bt(&$par){		
		return '<div style="margin-top:8px; float:none; "><a href="'.(isset($par['addr'])?$par['addr']:'').'">&lt;&lt; Назад</a></div>';
}}
?>